 ...
 */
export async function generateWithComfyUI(params) {
    try {
    ...
    for (const [nodeId, node] of Object.entries(workflow)) {
        ...
        if (node.class_type === 'SVD_img2vid_Conditioning') {
            if (params.frames !== undefined)
                node.inputs.video_frames = params.frames;
            if (params.fps !== undefined)
                node.inputs.fps = params.fps;
            if (params.motionScale !== undefined)
                node.inputs.motion_bucket_id = params.motionScale;
        }
    }
}
/**
