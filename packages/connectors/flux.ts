...
export interface FluxGenerateParams {
  ...
  imagePromptStrength?: number; // 0-1
  raw?: boolean; // Ultra Mode (photorealism)
 ...
 */
async function loadImageAsBase64(source: string): Promise<string> {
  // URL
 ...
 */
export async function generateWithFlux(params: FluxGenerateParams): Promise<FluxResult> {
 ...
 * Polling результата
 */
async function pollResult(requestId: string, apiKey: string): Promise<{ success: boolean; imageUrl?: string; error?: string }> {
